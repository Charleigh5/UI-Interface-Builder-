    ...
    return (
      ...
      >
        ...
        <div
          ...
          onDoubleClick={() => !component.isLocked && setIsEditing(true)}
        >
          {isEditing ? (
            <input
              type="text"
            ...
            <p className="text-sm truncate font-medium text-slate-700 dark:text-slate-200">
              {component.label}
            </p>
          )}
        </div>
...
export const ComponentList: React.FC = () => {
  ...
  const handleDelete = (id: string) => () => deleteComponent(id);

  const handleSelect = (id: string) => (e: React.MouseEvent<HTMLDivElement>) =>
    selectComponent(id, e.shiftKey);

  const handleToggleLock = (id: string) => () => toggleLock(id);
  ...
  const renderTree = (ids: string[], level: number): React.ReactNode[] =>
    ids.map(id => {
      const c = componentsById.get(id);
      if (!c) return null;
      const children =
        c.type === 'group' && c.childIds
          ? renderTree(c.childIds, level + 1)
          : null;
      return (
