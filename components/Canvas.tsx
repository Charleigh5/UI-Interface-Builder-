...
export const Canvas = () => {
    const { state, dispatch, addComponent, addLibraryComponent, selectComponent, setViewTransform } = useContext(AppContext);
    const { components, selectedComponentIds, currentTool, theme, zoom, pan, drawingSettings } = state;
    ...
    const screenToWorld = useCallback((screenCoords: { x: number; y: number }): { x: number; y: number } => {
        return {
    ...
    const draw = useCallback(() => {
        ...
        if (action === 'drawing' && currentShape) {
            ...
            ctx.fillStyle = shapeFill ? (theme === 'dark' ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)') : 'transparent';
            ...
            if (type === 'circle') {
                ...
                ctx.stroke();
            } else { // rectangle, button, input, text, image
    ...
    }, [components, selectedComponentIds, theme, action, currentShape, drawnPaths, zoom, pan, penWidth, penOpacity, shapeFill]);
    ...
    const drawComponent = (ctx: CanvasRenderingContext2D, component: WireframeComponent) => {
        ctx.save();
        const { x, y, width, height, type, properties, rotation = 0, isLocked } = component;
        ...
        if (type === 'button') {
            ...
            ctx.fillStyle = properties.textColor || (theme === 'dark' ? '#f1f5f9' : '#1e293b');
        ...
        if (isLocked) {
            ...
            ctx.save();
            ctx.fillStyle = theme === 'dark' ? 'rgba(248, 250, 252, 0.5)' : 'rgba(51, 65, 85, 0.3)';
            ctx.strokeStyle = theme === 'dark' ? 'rgba(248, 250, 252, 0.5)' : 'rgba(51, 65, 85, 0.3)';
            ctx.lineWidth = lockSize * 0.15;
            
            // Body
            ctx.beginPath();
            ctx.roundRect(iconX, iconY + lockSize * 0.4, lockSize, lockSize * 0.6, lockSize * 0.1);
            ctx.fill();

            // Shackle
            ctx.beginPath();
            ctx.arc(iconX + lockSize / 2, iconY + lockSize * 0.4, lockSize * 0.3, Math.PI, 2 * Math.PI);
            ctx.stroke();
    ...
    const drawHandles = (ctx: CanvasRenderingContext2D, c: WireframeComponent) => {
        ...
        const scaledRotationOffset = ROTATION_HANDLE_OFFSET / zoom;
    ...
    const getActionUnderCursor = (worldCoords: { x: number, y: number }) => {
        ...
        for (const c of reversedComponents) {
            ...
            const handleChecks: { name: ResizeHandle | 'rot', x: number, y: number, w: number, h: number }[] = [
                ...
                { name: 'bm', x: x + width/2 - hs/2, y: y + height - hs, w: hs, h: hs },
                { name: 'ml', x: x, y: y + height/2 - hs/2, w: hs, h: hs },
                { name: 'mr', x: x + width - hs, y: y + height/2 - hs/2, w: hs, h: hs },
                { name: 'rot', x: x + width/2 - hs/2, y: y - scaledRotationOffset, w: hs, h: hs },
            ];
            
            for (const handle of handleChecks) {
                const hx = handle.x - hs/2;
                const hy = handle.y - hs/2;
                if (p.x >= hx && p.x <= hx + handle.w + hs && p.y >= hy && p.y <= hy + handle.h + hs) {
                    return { action: handle.name === 'rot' ? 'rotating' : 'resizing', componentId: c.id, handle: handle.name };
                }
            }
        }
    ...
    const handleMouseDown = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
        ...
        if (currentTool === 'select') {
            if (actionUnderCursor) {
                ...
                setActiveResizeHandle(actionUnderCursor.handle as ResizeHandle | 'rot' | null);
                ...
                const effectivelySelected = components.filter(c => state.allEffectivelySelectedIds.has(c.id));
                effectivelySelected.forEach(c => {
                    original.set(c.id, c);
                    offsets.set(c.id, { dx: worldCoords.x - c.x, dy: worldCoords.y - c.y });
                });
            ...
            setCurrentShape({
                ...
                x: worldCoords.x, y: worldCoords.y, width: 0, height: 0,
                properties: getDefaultProperties(currentTool as any, theme),
            });
        }
    }, [currentTool, components, selectedComponentIds, screenToWorld, getActionUnderCursor, selectComponent, state.allEffectivelySelectedIds, theme]);
    ...
    const handleMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
        ...
        if (action === 'none') {
            ...
            if (actionUnderCursor) {
                const component = components.find(c => c.id === actionUnderCursor.componentId);
                if (actionUnderCursor.action === 'resizing' || actionUnderCursor.action === 'rotating') {
                    setCursor(getCursorForHandle(actionUnderCursor.handle!, component?.rotation));
        ...
        if (action === 'drawing') {
            ...
            setViewTransform({ pan: { x: pan.x + e.movementX, y: pan.y + e.movementY } });
        } else if (action === 'moving') {
            state.allEffectivelySelectedIds.forEach(id => {
                const original = originalComponents.get(id);
                const offset = moveOffsets.get(id);
                if (original && offset) {
                    dispatch({ type: 'UPDATE_COMPONENT', payload: { id, updates: { x: worldCoords.x - offset.dx, y: worldCoords.y - offset.dy } } });
                }
            });
        } else if (action === 'resizing' && activeResizeHandle && originalComponents.size > 0) {
            const [id, original] = Array.from(originalComponents.entries())[0];
            ...
            if (activeResizeHandle.includes('l')) { newX += rdx; newWidth -= rdx; }
            if (activeResizeHandle.includes('r')) { newWidth += rdx; }
            if (activeResizeHandle.includes('t')) { newY += rdy; newHeight -= rdy; }
            if (activeResizeHandle.includes('b')) { newHeight += rdy; }
            ...
            dispatch({ type: 'UPDATE_COMPONENT', payload: { id, updates: { x: finalX, y: finalY, width: newWidth, height: newHeight } } });
            ...
            const currentAngle = Math.atan2(worldCoords.y - center.y, worldCoords.x - center.x) * 180 / Math.PI;
            const originalRotation = original.rotation || 0;
            let newRotation = originalRotation + (currentAngle - startAngle);
            if(e.shiftKey) newRotation = Math.round(newRotation / 15) * 15;
            dispatch({ type: 'UPDATE_COMPONENT', payload: { id, updates: { rotation: newRotation } } });
        }
    }, [action, startPoint, currentShape, currentTool, screenToWorld, getActionUnderCursor, components, pan, zoom, setViewTransform, dispatch, moveOffsets, originalComponents, activeResizeHandle, state.allEffectivelySelectedIds]);
